...
class YOLOFireFlameDataset(Dataset):
    ...
    def load_dataset(self):
        images_dir = os.path.join(self.root, 'train/images')
        labels_dir = os.path.join(self.root, 'train/labels')
...
# 데이터셋 및 데이터로더 설정
path = dataset_path.fire_image_yolo
dataset = YOLOFireFlameDataset(root=path, transform=transform)
...
checkpoint_name = 'checkpoint_epoch_0.pth'
...
for epoch in range(start_epoch, num_epochs):
    for i, data in enumerate(tqdm(dataloader, desc=f'Epoch {epoch}/{num_epochs}'), 0):
        netD.zero_grad()
