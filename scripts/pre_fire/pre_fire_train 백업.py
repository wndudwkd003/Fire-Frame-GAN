...
noise_image_path = dataset_path.noise_image
...
for i in range(image_select_maxnum):
    ...
    # noisy_clean_image_float = noisy_clean_image.astype(np.float32)
    # flame_patch_image_float = flame_patch_image.astype(np.float32)
    
    # 두 이미지를 결합하여 6채널 이미지 생성
    # combined_6ch_image = np.concatenate((noisy_clean_image_float, flame_patch_image_float), axis=2)
